...
const templates: Record<string, FlowData> = {
  ...
  assistencia: {
    ...
    connections: [
      ...
      { id: "ass-conn-3", start: "ass-1", end: "ass-4", type: "negative" as ConnectionType, sourceHandle: "negative" },
      { id: "ass-conn-4", start: "ass-2", end: "ass-5", type: "positive" as ConnectionType, sourceHandle: "positive" }
    ]
  }
};
...
const FlowEditor: React.FC<FlowEditorProps> = ({ initialData = initialFlowData }) => {
  ...
  const [nodes, setNodes, onNodesChange] = useNodesState<Node<FlowCard>>([]);
  const [edges, setEdges, onEdgesChange] = useEdgesState<Edge<FlowConnection>>([]);
  ...
  // Create type-safe onSelect and onClose handlers for CardTypeSelector
  const handleCardTypeSelect = (type: CardType) => {
    console.log(`Selected card type: ${type}`);
  };

  const handleCardSelectorClose = () => {
    console.log("Card selector closed");
  };
