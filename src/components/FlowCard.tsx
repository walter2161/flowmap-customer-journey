...
const defaultProfile: AssistantProfile = {
  ...
  avatar: "",
  guidelines: "Este assistente deve ser cordial e respeitoso. Deve fornecer informações precisas e úteis. Não deve usar linguagem inapropriada ou compartilhar informações sensíveis. Não deve usar linguagem inapropriada ou compartilhar informações sensíveis.",
  scriptGuidelines: [
...
const FlowCardComponent: React.FC<FlowCardProps> = ({ data, selected }) => {
  ...
  const handleSaveProfile = () => {
    if (profile) {
      ...
      // Fix: Create a CustomEvent with proper type and initialization
      const event = new CustomEvent<AssistantProfile>('profileUpdated', { detail: profile });
            ...
            {fields.imoveis ? (
              <ScrollArea className="h-[120px]">
                <Table className="w-full">
                  <TableHeader>
                    <TableRow>
                      <TableHead className="text-xs">Descrição</
